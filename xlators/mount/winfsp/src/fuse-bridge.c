...
fuse_invalidate_entry (xlator_t* this, uint64_t fuse_ino)
{
        ...
        list_for_each_entry (dentry, &inode->dentry_list, inode_list)
        {
                ...
                node->is_entry = 1;
                node->unique = 0;
                node->error = FUSE_NOTIFY_INVAL_ENTRY;
                ...
                if (dentry->parent) {
                        ...
                        fuse_log_eh (
                          this, "Invalidated entry %s(nodeid: %" PRIu64 ")",
                          dentry->name, node->parent);
                }
        }
...
fuse_invalidate_inode (xlator_t* this, uint64_t fuse_ino)
{
        ...
        node->is_entry = 0;
        node->unique = 0;
        node->error = FUSE_NOTIFY_INVAL_INODE;
        node->ino = fuse_ino;
...
static void
fuse_lookup_continue (winfsp_msg_t* msg, xlator_t* this, uint64_t nodeid, char* bname);
                ...
                struct iatt* buf, dict_t* xdata)
{
        ...
        winfsp_msg_t* stub = NULL;
        winfsp_lookup_t* lookup_args = NULL;
        char* bname = NULL;
                 ...
                 struct iatt* stat, dict_t* dict, struct iatt* postparent)
{
        ...
        if (op_ret == -1 && state->is_revalidate == 1) {
                ...
                STACK_WIND (frame, fuse_lookup_cbk, prev->this,
                            prev->this->fops->lookup, &state->loc,
                            state->xdata);
                return 0;
        }
...
fuse_lookup_continue (winfsp_msg_t* msg, xlator_t* this, uint64_t nodeid, char* bname)
{
        ...
        ctx = get_fuse_header_in ();
        INIT_FUSE_HEADER (finh, get_fuse_op_unique (), msg->type, ctx);
        FILL_STATE (msg, this, finh, NULL, state);
        finh->nodeid = nodeid;
        state->stub = msg;

        fuse_resolve_entry_init (state, &state->resolve, nodeid, bname);

        gf_log ("glusterfs-fuse", GF_LOG_DEBUG,
                "%" PRIu64 ": LOOKUP nodeid:%ld, name: %s",
                msg->unique, finh->nodeid, bname);

        fuse_resolve_and_resume (state, fuse_lookup_resume);
}

static void
fuse_lookup (xlator_t* this, winfsp_msg_t* msg)
{
        ...
        if (args->nodeid != 0) {
                gf_log ("glusterfs-fuse", GF_LOG_DEBUG,
                        "%" PRIu64 ": LOOKUP nodeid:%ld, name: %s",
                        msg->unique, args->nodeid, args->path);

                FILL_STATE (msg, this, finh, NULL, state);
                finh->nodeid = args->nodeid;

                bname = args->path + args->lookup_offset;
                ...
                gf_log ("glusterfs-fuse", GF_LOG_DEBUG,
                        "%" PRIu64 ": LOOKUP fullpath: %s",
                        msg->unique, args->path);

                FILL_STATE (msg, this, finh, NULL, state);
                state->stub = msg;

                subdir = args->path;
                inode = fuse_inode_from_path (this, subdir, state->itable);
                while (inode == NULL) {
                        endp = strrchr (subdir, '/');
                        if (endp > subdir)
                                *endp = '\0';
                ...
                if (endp == NULL) {
                        ...
                        args->lookup_offset = endp - args->path + 1 + strlen (bname) + 1;
...
static void
fuse_forget (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_forget_t* args = (winfsp_forget_t*)msg->args;
        fuse_in_header_t* finh = msg->finh;
...
static void
fuse_batch_forget (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_batch_forget_t* args = (winfsp_batch_forget_t*)msg->args;
        fuse_in_header_t* finh = msg->finh;
        int i;
...
static void
fuse_getattr (xlator_t* this, winfsp_msg_t* msg)
{
             ...
             int32_t op_errno, fd_t* fd, dict_t* xdata)
{
        ...
        winfsp_msg_t* stub = NULL;
        winfsp_open_t* args = NULL;

        priv = this->private;
        state = frame->root->state;
        finh = state->finh;
        stub = state->stub;
        args = (winfsp_open_t*)stub->args;
        ...
        if (op_ret >= 0) {
                ...
                /*
                   if (winfsp_send_result(this, finh, &foo) == ENOENT) {
                        gf_log("glusterfs-fuse", GF_LOG_DEBUG,
                               "open(%s) got EINTR",
                                state->loc.path);
                  ...
                  struct iatt* statpost, dict_t* xdata)
{
        ...
        if (op_ret == 0) {
                ...
                winfsp_send_err (state->this, state->stub, op_errno);
                op_done = 1;
        }
...
static void
fuse_setattr (xlator_t* this, winfsp_msg_t* msg)
{
                      ...
                      int32_t op_ret, int32_t op_errno, dict_t* xdata)
{
        ...
        if (op_ret == 0) {
                ...
                winfsp_send_err (state->this, state->stub, 0);
        } else {
                gf_log ("glusterfs-fuse",
                 ...
                 struct iatt* postparent, dict_t* xdata)
{
        ...
        if (op_ret == 0) {
                ...
                winfsp_send_err (this, state->stub, 0);
        } else {
                if (GF_IGNORE_IF_GSYNCD_SAFE_ERROR (frame, op_errno)) {
                        gf_log (
                          "glusterfs-fuse",
...
static void
fuse_access (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_access_t* args = (winfsp_access_t*)msg->args;
        fuse_in_header_t* finh = msg->finh;
        fuse_state_t* state = NULL;
...
static void
fuse_readlink (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_readlink_t* args = (winfsp_readlink_t*)msg->args;
        fuse_in_header_t* finh = msg->finh;
        fuse_state_t* state = NULL;
        int32_t ret = -1;
...
static void
fuse_mknod (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_mknod_t* args = (winfsp_mknod_t*)msg->args;
...
static void
fuse_mkdir (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_mkdir_t* args = (winfsp_mkdir_t*)msg->args;
...
static void
fuse_unlink (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_unlink_t* args = (winfsp_unlink_t*)msg->args;
        fuse_in_header_t* finh = NULL;
        fuse_state_t* state = NULL;
        char* name = NULL;
        char* path = NULL;
...
#ifdef NEVER /* the following code can cause inode_unref() crash because of    \
                (ref=0) */
...
static void
fuse_rmdir (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_rmdir_t* args = (winfsp_rmdir_t*)msg->args;
        fuse_in_header_t* finh = NULL;
        fuse_state_t* state = NULL;
        char* name = NULL;
        char* path = NULL;
...
fuse_symlink_resume (fuse_state_t* state)
{
        ...
        FUSE_FOP (state, fuse_newentry_cbk, GF_FOP_SYMLINK, symlink,
                  state->name, &state->loc, state->umask, state->xdata);
}

static void
fuse_symlink (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_symlink_t* args = (winfsp_symlink_t*)msg->args;
        fuse_in_header_t* finh = NULL;
        fuse_state_t* state = NULL;
        char* name = NULL;
        char* path = NULL;
                 ...
                 dict_t* xdata)
{
        ...
        fuse_log_eh (
          this, "op_ret: %d, op_errno: %d, %" PRIu64 ": %s() "
...
static void
fuse_rename (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_rename_t* args = (winfsp_rename_t*)msg->args;
        fuse_in_header_t* finh = NULL;
        fuse_state_t* state = NULL;
...
static void
fuse_link (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_link_t* args = (winfsp_link_t*)msg->args;
        fuse_in_header_t* finh = NULL;
        fuse_state_t* state = NULL;
        inode_t* inode = NULL;
        char* oldname = NULL;
        ...
        fuse_resolve_entry_init (state, &state->resolve2, finh->nodeid, bname);

        fuse_resolve_entry_init (state, &state->resolve, newnodeid, newname);
                 ...
                 struct iatt* postparent, dict_t* xdata)
{
        ...
        if (stub->type == FUSE_CREATE) {
                args = (winfsp_create_t*)stub->args;
        }
...
static void
fuse_create (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_create_t* args = (winfsp_create_t*)msg->args;
        fuse_in_header_t* finh = msg->finh;
...
static void
fuse_open (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_open_t* args = (winfsp_open_t*)msg->args;
        fuse_in_header_t* finh = NULL;
        fuse_state_t* state = NULL;
                ...
                dict_t* xdata)
{
        ...
        fuse_in_header_t* finh = NULL;
        winfsp_msg_t* stub = NULL;
        winfsp_read_t* args = NULL;
        int len = 0;

        state = frame->root->state;
        finh = state->finh;
        stub = state->stub;
        args = (winfsp_read_t*)stub->args;
        ...
        if (op_ret >= 0) {
                ...
                if (len > args->size) {
                        ...
                        winfsp_send_result (this, stub, len);
                }
        } else {
                gf_log ("glusterfs-fuse", GF_LOG_WARNING,
...
static void
fuse_read (xlator_t* this, winfsp_msg_t* msg)
{
                 ...
                 struct iatt* postbuf, dict_t* xdata)
{
        ...
        if (op_ret >= 0) {
                ...
                winfsp_send_result (this, state->stub, op_ret);
        } else {
                gf_log ("glusterfs-fuse", GF_LOG_WARNING,
...
static void
fuse_write (xlator_t* this, winfsp_msg_t* msg)
{
...
static void
fuse_flush (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_flush_t* args = (winfsp_flush_t*)msg->args;
...
static void
fuse_release (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_flush_t* args = (winfsp_release_t*)msg->args;
...
static void
fuse_fsync (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_fsync_t* args = (winfsp_fsync_t*)msg->args;
        fuse_in_header_t* finh = msg->finh;
        fuse_state_t* state = NULL;
        fd_t* fd = NULL;
        fuse_private_t* priv = NULL;
...
static void
fuse_opendir (xlator_t* this, winfsp_msg_t* msg)
{
                     ...
                     dict_t* xdata)
{
        ...
        if (max_size == 0) {
                winfsp_send_result (this, stub, -1);
                goto out;
        }
        ...
        list_for_each_entry (entry, &entries->list, list)
        {
                ...
                if (strcmp (fde->name, ".") != 0 &&
                    strcmp (fde->name, "..") != 0)
                        winfsp_lookup (this, finh->nodeid, fde->name);
...
fuse_lookup_dir_resume (fuse_state_t* state)
{
        ...
        FUSE_FOP (state, fuse_lookup_dir_cbk, GF_FOP_READDIR, readdir,
                  state->fd, state->size, state->off, state->xdata);
}

static void
fuse_lookup_dir (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_lookup_dir_t* args = (winfsp_lookup_dir_t*)msg->args;
        fuse_in_header_t* finh = msg->finh;
        fuse_state_t* state = NULL;
        inode_t* inode = NULL;
        fd_t* fd = NULL;
...
{
        winfsp_readdirp_t* args = (winfsp_readdirp_t*)msg->args;
        fuse_in_header_t* finh = msg->finh;
        fuse_state_t* state = NULL;
        fd_t* fd = NULL;
...
fuse_fallocate_resume (fuse_state_t* state)
{
        gf_log ("glusterfs-fuse", GF_LOG_TRACE,
                "%" PRIu64
                ": FALLOCATE (%p, flags=%d, size=%zu, offset=%" PRId64 ")",
        ...
        if (state->flags & FALLOC_FL_PUNCH_HOLE)
                ...
                FUSE_FOP (state, fuse_fallocate_cbk, GF_FOP_FALLOCATE,
                          fallocate, state->fd,
                          (state->flags & FALLOC_FL_KEEP_SIZE), state->off,
                          state->size, state->xdata);
}

static void
fuse_fallocate (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_fallocate_t* args = (winfsp_fallocate_t*)msg->args;
        fuse_in_header_t* finh = msg->finh;
        fuse_state_t* state = NULL;
...
static void
fuse_releasedir (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_releasedir_t* args = (winfsp_releasedir_t*)msg->args;
        fd_t* activefd = NULL;
        uint64_t val = 0;
        int ret = 0;
        fuse_state_t* state = NULL;
...
static void
fuse_fsyncdir (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_fsyncdir_t* args = (winfsp_fsyncdir_t*)msg->args;
        fuse_in_header_t* finh = msg->finh;
        fuse_state_t* state = NULL;
        fd_t* fd = NULL;
                 ...
                 dict_t* xdata)
{
        ...
        winfsp_statfs_t* args = NULL;

        state = frame->root->state;
        priv = this->private;
        finh = state->finh;
        args = (winfsp_statfs_t*)state->stub->args;
...
static void
fuse_statfs (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_statfs_t* args = (winfsp_statfs_t*)msg->args;
        fuse_in_header_t* finh = msg->finh;
        fuse_state_t* state = NULL;
...
fuse_setxattr_resume (fuse_state_t* state)
{
        ...
        if (state->fd) {
                ...
                FUSE_FOP (state, fuse_setxattr_cbk, GF_FOP_SETXATTR, setxattr,
                          &state->loc, state->xattr, state->flags,
                          state->xdata);
        }
}

static void
fuse_setxattr (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_setxattr_t* args = msg->args;
        fuse_in_header_t* finh = msg->finh;
        fuse_state_t* state = NULL;
        ...
        if (fsi->position) {
                ...
                winfsp_send_err (state->this, state->stub, EINVAL);
                FREE (finh);
                return;
        }
#endif
        ...
        if (fuse_ignore_xattr_set (priv, args->name)) {
                (void)winfsp_send_err (state->this, state->stub, 0);
                free_fuse_state (state);
                return;
        }
        ...
        if (!priv->acl) {
                if ((strcmp (args->name, POSIX_ACL_ACCESS_XATTR) == 0) ||
                    (strcmp (args->name, POSIX_ACL_DEFAULT_XATTR) == 0)) {
                        winfsp_send_err (state->this, state->stub, EOPNOTSUPP);
                        free_fuse_state (state);
                        return;
                }
        }
        ...
        if (ret >= 0) {
                winfsp_send_err (state->this, state->stub, ret);
                free_fuse_state (state);
                return;
        }
        ...
        if (!strcmp (GFID_XATTR_KEY, args->name) ||
            !strcmp (GF_XATTR_VOL_ID_KEY, args->name)) {
                winfsp_send_err (state->this, state->stub, EPERM);
                free_fuse_state (state);
                return;
        }
...
static void
send_fuse_xattr (xlator_t* this, winfsp_msg_t* stub, const char* value,
                 size_t size, size_t expected)
{
        ...
        if (size > GLUSTERFS_XATTR_LEN_MAX)
                winfsp_send_err (this, stub, E2BIG);
        else if (expected) {
                /* if callback for getxattr and asks for value */
                if (size > expected)
                        /* reply would be bigger than
                         * what was asked by kernel */
                        winfsp_send_err (this, stub, ERANGE);
                else
                        // winfsp_send_result(this, stub, (void*)value, size);
                        winfsp_send_result (this, stub, 0);
        } else {
                fgxo.size = size;
                // winfsp_send_result(this, stub, &fgxo);
                winfsp_send_result (this, stub, 0);
        }
}
                ...
                int32_t op_ret, int32_t op_errno, dict_t* dict, dict_t* xdata)
{
        ...
        if (op_ret >= 0) {
                ...
                if (state->name) {
                        ...
                        send_fuse_xattr (this, state->stub, value, len,
                                         state->size);
                } /* if(state->name)...else */
        } else {
...
static void
fuse_getxattr (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_getxattr_t* args = (winfsp_getxattr_t*)msg->args;
        fuse_in_header_t* finh = msg->finh;
        fuse_state_t* state = NULL;
        fuse_private_t* priv = NULL;
...
static void
fuse_listxattr (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_listxattr_t* args = (winfsp_listxattr_t*)msg->args;
        fuse_in_header_t* finh = NULL;
        fuse_state_t* state = NULL;
...
static void
fuse_removexattr (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_removexattr_t* args = (winfsp_removexattr_t*)msg->args;
        fuse_in_header_t* finh = msg->finh;
        fuse_state_t* state = NULL;
...
static void
fuse_getlk (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_setlk_t* args = (winfsp_setlk_t*)msg->args;
        fuse_in_header_t* finh = msg->finh;
        fuse_state_t* state = NULL;
        fd_t* fd = NULL;
        ...
        convert_fuse_file_lock (&args->lk, &state->lk_lock,
                                args->fi->lock_owner);
...
static void
fuse_setlk (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_setlk_t* args = (winfsp_setlk_t*)msg->args;
        fuse_in_header_t* finh = msg->finh;
        fuse_state_t* state = NULL;
...
static void
fuse_init (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_init_t* args = (winfsp_init_t*)msg->args;
        ...
        ret = gf_thread_create (&messenger, NULL, notify_kernel_loop, this, NULL);
        if (ret != 0) {
                gf_log ("glusterfs-fuse", GF_LOG_ERROR,
...
static void
fuse_enosys (xlator_t* this, winfsp_msg_t* msg)
{
                      ...
                      xlator_t* old_subvol, xlator_t* new_subvol)
{
        ...
        gf_log ("glusterfs-fuse", GF_LOG_INFO,
                ...
                "(old-subvolume:%s-%d new-subvolume:%s-%d)",
                basefd, newfd, uuid_utoa (basefd->inode->gfid),
                 ...
                 xlator_t* new_subvol)
{
        ...
        if (oldfd->inode->table->xl == old_subvol) {
                if (IA_ISDIR (oldfd->inode->ia_type))
                        ret =
                          syncop_fsyncdir (old_subvol, oldfd, 0, NULL, NULL);
                else
        ...
        ret =
          fuse_migrate_fd_open (this, basefd, oldfd, old_subvol, new_subvol);
        if (ret < 0) {
                gf_log (this->name, GF_LOG_WARNING,
                        ...
                        xlator_t* new_subvol)
{
        ...
        if (fdentries != NULL) {
                for (i = 0; i < count; i++) {
                        ...
                        ret =
                          fuse_migrate_fd (this, fd, old_subvol, new_subvol);
...
static void
fuse_autorelease (xlator_t* this, winfsp_msg_t* msg)
{
        winfsp_autorelease_t* args = (winfsp_autorelease_t*)msg->args;
        winfsp_cleanup_req (args->msg);
        winfsp_cleanup_req (msg);
}

static void
fuse_waitmsg (xlator_t* this, winfsp_msg_t* msg)
{
        ...
        for (;;) {
                        ...
                        while ((msg = get_next_msg (priv)) == NULL) {
                                ...
                                list_for_each_entry(tt, &priv->wait_list, list) {
                                        gf_msg(this->name, GF_LOG_INFO, 0, LG_MSG_POLL_IGNORE_MULTIPLE_THREADS,
                                               ...
                                               ((winfsp_waitmsg_t *)tt->args)->msg->unique);
                                }
#endif /* DEBUG */
                ...
                if (msg->type == FUSE_LOOKUP) {
                        ...
                        gf_msg (this->name, GF_LOG_INFO, 0,
                                ...
                                msg->type, msg->unique, args->nodeid,
                                args->path ? args->path : "",
                                args->path ? args->path + args->lookup_offset : "");
#endif /* NEVER */
        ...
        switch (event) {
                ...
                case GF_EVENT_CHILD_CONNECTING: {
                        ...
                        if (start_thread) {
                                ...
                                pthread_getschedparam (private->fuse_thread,
                                                       &policy, &param);

#ifdef NEVER
                                param.sched_priority = 1;
                                pthread_setschedparam (private->fuse_thread,
                                                       SCHED_FIFO,
...
static void
fuse_dumper (xlator_t* this, winfsp_msg_t* msg)
{
        int ret = -1;
        fuse_private_t* priv = NULL;
        priv = this->private;
        ...
        ret =
          gf_asprintf (&buf, "fuse received message %p type: %d, unique: %lu\n",
...
init (xlator_t* this_xl)
{
        ...
        */
        ...
        if (stat (value_string, &stbuf) != 0) {
                if (errno == ENOENT) {
                        ...
                        win_mount_path =
                          create_winpath_from_cygpath (value_string);
        ...
        */
        ...
        ret = gf_thread_create (&priv->mount_thread, NULL, winfsp_mount_proc,
                                mnt_data, NULL);
        if (ret != 0) {
                gf_log (this_xl->name, GF_LOG_DEBUG,
...
struct xlator_cbks cbks = {.invalidate = fuse_invalidate_2,
