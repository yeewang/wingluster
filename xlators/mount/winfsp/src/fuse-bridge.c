        ...
        next_entry:
                ...
                pthread_mutex_lock(&rd_stub->mutex);
                {
                        winfsp_readdirp_item_t *msg =
                                (winfsp_readdirp_item_t *) GF_CALLOC(1, sizeof(winfsp_readdirp_item_t), gf_fuse_mt_char);
                        if (msg == NULL) {
                                gf_log("glusterfs-fuse", GF_LOG_DEBUG,
                                        "%" PRIu64 ": READDIRP => -1 (%s)", frame->root->unique,
                                        strerror(ENOMEM));
        ...
        if (op_ret >= 0) {
                ...
                if (state->name) {
                        ...
                        if (value_data) {
                                ...
                                winfsp_send_err(state->this, state->stub, ENODATA);
                        } /* if(value_data)...else */
                } else {
...
fuse_thread_proc(void* data)
{
        ...
        for (;;) {
                ...
                pthread_mutex_lock(&priv->msg_mutex);
                {
                        ...
                        msg =
                          list_first_entry(&priv->msg_list, winfsp_msg_t, list);
                        list_del_init(&msg->list);

                        if (msg->type != FUSE_AUTORELEASE &&
                            msg->type != FUSE_FORGET &&
                            msg->type != FUSE_BATCH_FORGET) {
                                waitmsg = GF_CALLOC(1, sizeof(winfsp_msg_t) +
                                        sizeof(winfsp_waitmsg_t), gf_fuse_mt_winfsp_msg_t);
                                if (waitmsg == NULL) {
