...
static int fuse_resolve_all (fuse_state_t* state);
...
fuse_fd_ctx_t* fuse_fd_ctx_get (xlator_t* this, fd_t* fd);
                        ...
                        struct iatt* postparent)
{
        ...
        link_inode =
          inode_link (inode, resolve_loc->parent, resolve_loc->name, buf);
        if (link_inode == inode)
...
fuse_resolve_entry (fuse_state_t* state)
{
        ...
        resolve_loc->inode =
          inode_grep (state->itable, resolve->parhint, resolve->bname);
        ...
        FUSE_FOP (state, fuse_resolve_entry_cbk, GF_FOP_LOOKUP, lookup,
                  resolve_loc, NULL);
...
fuse_resolve_gfid (fuse_state_t* state)
{
        ...
        FUSE_FOP (state, fuse_resolve_gfid_cbk, GF_FOP_LOOKUP, lookup,
                  resolve_loc, NULL);
...
#define FUSE_FD_GET_ACTIVE_FD(activefd, basefd)                                \
        do {                                                                   \
                LOCK (&basefd->lock);                                          \
                {                                                              \
                        activefd = basefd_ctx->activefd ? basefd_ctx->activefd \
                                                        : basefd;              \
                        if (activefd != basefd) {                              \
                                fd_ref (activefd);                             \
                        }                                                      \
...
fuse_resolve_fd (fuse_state_t* state)
{
        ...
        if (fd_migration_error) {
                ...
                fd_migration_error =
                  fuse_migrate_fd_error (state->this, basefd);
                fd_unref (activefd);
...
fuse_gfid_set (fuse_state_t* state)
{
        ...
        ret = dict_set_static_bin (state->xdata, "gfid-req", state->gfid,
                                   sizeof (state->gfid));
out:
        return ret;
}
