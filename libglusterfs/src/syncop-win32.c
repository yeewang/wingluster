                 ...
                 synctask_cbk_t cbk, call_frame_t *frame, void *opaque)
{
        ...
        if (!cbk) {
                uv_mutex_init (&newtask->mutex);
                uv_cond_init (&newtask->cond);
                newtask->done = 0;
        }
...
syncenv_task (struct syncproc *proc)
{
        ...
        uv_mutex_lock (&env->mutex);
        {
                while (list_empty (&env->runq)) {
                        ret = uv_cond_timedwait (&env->cond, &env->mutex,
                                                 SYNCPROC_IDLE_TIME * 1e9);
...
syncenv_destroy (struct syncenv *env)
{
        ...
        {
                env->destroy = 1;
                /* This broadcast will wake threads in uv_cond_wait
                 * in syncenv_task
                 */
                uv_cond_broadcast (&env->cond);
                 ...
                 */
                while (env->procs != 0) {
                        uv_cond_wait (&env->cond, &env->mutex);
                }
        }
        uv_mutex_unlock (&env->mutex);

        uv_mutex_destroy (&env->mutex);
        uv_cond_destroy (&env->cond);
...
syncenv_new (size_t stacksize, int procmin, int procmax)
{
	...
	if (procmin > procmax)
        ...
        uv_mutex_init (&newenv->mutex);
        uv_cond_init (&newenv->cond);
...
synclock_init (synclock_t *lock, lock_attr_t attr)
{
        ...
        return uv_mutex_init (&lock->guard);
}
...
__synclock_lock (struct synclock *lock)
{
        ...
        while (lock->lock) {
                if (task) {
                        ...
                        uv_mutex_unlock (&lock->guard);
                        synctask_yield (task);
                        /* task is removed from waitq in unlock,
                         * under lock->guard.*/
                        uv_mutex_lock (&lock->guard);
                } else {
                        /* called by a non-synctask */
                        uv_cond_wait (&lock->cond, &lock->guard);
                }
        }
...
syncbarrier_init (struct syncbarrier *barrier)
{
	...
	uv_cond_init (&barrier->cond);
	barrier->count = 0;
	INIT_LIST_HEAD (&barrier->waitq);

	return uv_mutex_init (&barrier->guard);
}
	...
	while (barrier->count < waitfor) {
		if (task) {
			...
			list_add_tail (&task->waitq, &barrier->waitq);
                        uv_mutex_unlock (&barrier->guard);
                        synctask_yield (task);
                        uv_mutex_lock (&barrier->guard);
		} else {
			/* called by a non-synctask */
			uv_cond_wait (&barrier->cond, &barrier->guard);
		}
	}
