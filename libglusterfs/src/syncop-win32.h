...
struct synctask {
        ...
        uv_mutex_t          mutex; /* for synchronous spawning of synctask */
        uv_cond_t           cond;
        int                 done;
...
struct synclock {
        uv_mutex_t          guard; /* guard the remaining members, pair @cond */
...
#define __yawn(args) do {                                       \
        ...
        if (args->task)                                         \
            break;                                              \
        uv_mutex_init (&args->mutex);                           \
        uv_cond_init (&args->cond);                             \
        args->done = 0;                                         \
        } while (0)
...
#define __wake(args) do {                                       \
        if (args->task) {                                       \
                ...
                uv_mutex_lock (&args->mutex);                   \
                {                                               \
                        args->done = 1;				\
                        uv_cond_signal (&args->cond);           \
                }                                               \
                uv_mutex_unlock (&args->mutex);                 \
        }                                                       \
        } while (0)
...
#define __yield(args) do {						\
	if (args->task) {				                \
		...
		{							\
			while (!args->done)				\
				uv_cond_wait (&args->cond,		\
					      &args->mutex);	        \
		}							\
		uv_mutex_unlock (&args->mutex);			        \
		uv_mutex_destroy (&args->mutex);		 	\
		uv_cond_destroy (&args->cond);		         	\
	}								\
	} while (0)
